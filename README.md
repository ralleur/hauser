...
normal installation path; `v0.8.1` is current. The first installation by
...
The image `ghcr.io/ralleur/hauser:v0.8.1` is published only after the
matching public beta tag passes the release workflow. Tagged releases also
publish the plain `0.8.1` tag, which the Home Assistant Supervisor
