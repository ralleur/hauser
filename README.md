...
normal installation path; `v0.4.0-beta.9` is current. The first installation by
...
The image `ghcr.io/ralleur/hauser:v0.4.0-beta.9` is published only after the
matching public beta tag passes the release workflow. Tagged releases also
publish the plain `0.4.0-beta.9` tag, which the Home Assistant Supervisor
