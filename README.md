...
normal installation path; `v0.20.5` is current. The first installation by
...
The image `ghcr.io/ralleur/hauser:v0.20.5` is published only after the
matching public beta tag passes the release workflow. Tagged releases also
publish the plain `0.20.5` tag, which the Home Assistant Supervisor
