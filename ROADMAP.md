...
| Beta stabilisation | `v0.9.1` and later `v0.x` | An external real-home installation is done ([#7](https://github.com/ralleur/hauser/issues/7)); a release-to-release upgrade, backup/restore and rollback on an external installation are still outstanding before RC |
| Release candidate | `v0.9.1-rc.1` | Configuration contract frozen; clean install, upgrade and rollback green; only release blockers remain |
