...
describe('phone reminder freshness status', () => {
  it('keeps reminder rows visible while marking failed refreshes as stale', () => {
    expect(phoneReminders).toMatch(
      /\{#if reminders\.error\}\s*<p class="phone-calendar-status is-error" role="status">Letzte bekannte Erinnerungen<\/p>\s*\{\/if\}/,
    );
    expect(phoneReminders).toMatch(
  ...
  it('guards the only error status directly so null renders no status', () => {
    const staleStatus = '<p class="phone-calendar-status is-error" role="status">Letzte bekannte Erinnerungen</p>';
